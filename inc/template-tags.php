...
if ( ! function_exists( 'fabio_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function fabio_posted_on() {
	?>
...
if ( ! function_exists( 'fabio_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function fabio_posted_by() {
		global $post;
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( '%s', 'post author', 'fabio' ),
...
if ( ! function_exists( 'fabio_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function fabio_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'fabio' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'fabio' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'fabio' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'fabio' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'fabio' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'fabio' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'fabio_entry_meta_footer' ) ) :
		/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 ...
	 * @return void
	 */
	function fabio_entry_meta_footer() {
     global $post;
	$readmore_butotn_text = fabio_opt('read_more_text_button', 'Read More');
...
function fabio_is_comment_by_post_author( $comment = null ) {
 ...
 */
 ...
 if(!function_exists('fabio_logo')) {
	...
	*/
		...
		function fabio_logo () {
			...
			$logo_option_url = fabio_IMAGES.'/default_logo/logo.svg';
			$sticky_logo_option = fabio_IMAGES.'/default_logo/logo_sticky.svg';
			$sticky_ratina_sticky_url = '';
			$sticky_ratina_attr = '';
			$ratena_url = '';
			$ratena_attr = '';
			$logo_alt = 'fabio default logo';
			$logo_alt_sticky = 'fabio default sticky logo';

			if(class_exists( 'Redux' ) ) {

				$logo = fabio_opt('logo');
				$logo_sticky = fabio_opt('sticky_logo');
				$logo_ratina = fabio_opt('retina_logo');
				$logo_ratina_sticky = fabio_opt('retina_sticky_logo');
  ...
  * display tag 
  */
 if(!function_exists('fabio_single_page_tag'))  {
	 function fabio_single_page_tag() {
		 if(has_tag() ) :
		 ?>
		 <div class="tagcloud">
            <?php the_tags(null, ''); ?>
...
if(!function_exists('fabio_about_author')) {
	
	function fabio_about_author() {
		global $post;
		$bio =  get_the_author_meta( 'description',$post->post_author);
		if(!empty($bio)):
      ?>
	  <div class="about-autheor">
		<h3 class="comment-reply-title"><?php echo esc_html__( 'About the Author', 'fabio' ); ?></h3>
