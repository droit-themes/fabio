...
function fabio_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'fabio_pingback_header' );

function fabio_continue_reading_text() {
...
function fabio_continue_reading_link_excerpt() {
	global $post;
	return ' ...';
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'fabio_continue_reading_link_excerpt' );
...
function fabio_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
...
if(!function_exists('fabio_can_show_post_thumbnail')) : 
/**
 ...
 * @return bool
 */
function fabio_can_show_post_thumbnail() {
	return apply_filters(
		'fabio_can_show_post_thumbnail',
 ...
 *  
 **/
add_filter( 'get_custom_logo', 'fabio_custom_logo_class' );


function fabio_custom_logo_class( $html ) {
...
//Comment Field Order
add_filter( 'comment_form_fields', 'fabio_comment_fields_custom_order' );
function fabio_comment_fields_custom_order( $fields ) {
...
if(!function_exists('fabio_wrapper_start')) {

	function fabio_wrapper_start( $sidebar = 'right', $column = 8 ) {
...
if(!function_exists('fabio_wrapper_end')) {

	function fabio_wrapper_end( $sidebar = 'right' ) {
		?>
		  </div>
		 <?php
			if($sidebar != 'full' && is_active_sidebar( 'blog_sidebar' )){
...
if ( !function_exists('fabio_header_footer_builder_id')){

    function fabio_header_footer_builder_id($posttype, $post_id) {
...
function get_themebuilder_Id( $id, $type = 'header' ) {
 ...
 return fabio_themebuilder_id($dispaly_list, $id);

}

if(!function_exists('_fabio_header')){
	function _fabio_header( $header_id ) {
	...
	add_action('fabio_banner_content
', '_fabio_header');
}

if(!function_exists('_fabio_footer')){
	function _fabio_footer( $footer_id ) {
		...
		if ( $footer_id != '' && class_exists( '\Elementor\Plugin' ) ) {
			...
			$footer_text = fabio_opt('footer_copyright_txt', 'Copyright &copy; 2022 <a href="https://droitthemes.com/">DroitThemes</a> | All rights reserved');
			?>
			<footer id="colophon" class="site-footer text-center">
				<div class="site-info container">
					<?php echo fabio_kses($footer_text); ?>
				</div><!-- .site-info -->
			</footer><!-- #colophon -->
			<?php
		}
	...
	add_action('_fabio_footer_content', '_fabio_footer');
}

if(!function_exists('fabio_banner_display')) {
	function fabio_banner_display( $banner_id, $display_on ) {
	...
	add_action('fabio_banner_content', 'fabio_banner_display', 10, 2);
}
...
if(!function_exists('fabio_themebuilder_id')){
function fabio_themebuilder_id($arr = [], $id='') {
  if(empty($arr)) {
	  return;
  }
  global $post;
